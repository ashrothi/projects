...
public class AOPLogging {
	...
	public void logAfterMappingHandler(JoinPoint joinPoint) {
		...
		try {
			...
			/**
			 * To Check if the Login API is Called than Password user_key and token will not
			 * be in Parameter list
			 */
			if (methodName.equals("oauthToken")) {
			...
			} else if (methodName.equals("desktopOauthToken")) {
				try {
					/**
					 * to get parameters in Array
					 */
				...
				try {
					if (argsParam != null) {
						...
						String userKeyValue = request.getHeader("user_key");
						if (tokenValue != null) {
							/**
							 * To get Token Value
							 */
							Token token = (Token) Constant.map.get(userKeyValue.toString());
						...
						for (String key : argsParam.keySet()) {
							/**
							 * To get rest of the parameter passed in calling respective API
							 */
							finalInputArg.append(key + " = " + argsParam.get(key) + " ,");
						}
						if (finalInputArg.length() > 0) {
							/**
							 * If size is more than than remove last , from the string
							 */
			...
			/**
			 * To Insert the logs in the database by executing log insert procedure
			 */
			/*
			 * barc_budde_audit_log_insert(controller_name,input_parameters,
			 * ip_address,url_accessed,user_key,user_id,application_key, organization_id)
			 */
			
			
			
			genericService.executeProcesure(null, proParam.get("28").toString(), controllerName.toString(),
