...
public class AccessRestApi {
	 ...
	 * @return Output string retrieved from API.
	 */
	public String callingRestAPIWithHeaders(String URL, HashMap<String, String> params, String Token) {
		...
		try {
			 ...
			 * Initialize the URL and create connection.
			 */
			FftLogger.logger.info(" Api Url:" + URL);
			URL url = new URL(URL);
			HttpURLConnection connection = (HttpURLConnection) url.openConnection();
			...
			connection.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");
			connection.setRequestProperty("charset", "utf-8");
			if (Token.equalsIgnoreCase("thirdParty")) {
				connection.setRequestProperty("token", getThirdPartyAuthToken());
			} else {
				connection.setRequestProperty("token", getAuthToken());
			}

			connection.setRequestProperty("user_id", FftEscalationApp.authUserName);
			connection.setRequestProperty("user_key", FftEscalationApp.userKey);
			...
			if (params.size() != 0) {
				...
				urlParams.deleteCharAt(urlParams.lastIndexOf("&"));
				FftLogger.logger.info(urlParams);
			}
			OutputStream outputStream = connection.getOutputStream();
			outputStream.write(urlParams.toString().getBytes());
			outputStream.flush();
			...
			if (connection.getResponseCode() != HttpURLConnection.HTTP_CREATED) {
				...
				FftLogger.logger.info("Response Code is:   " + connection.getResponseCode());
			} else {
				throw new RuntimeException("Failed : HTTP error code : " + connection.getResponseCode());
			}
	...
	public String getAuthToken() {
		...
		String authEngineUrl = FftEscalationApp.authApiUrl;
		...
		authEngineparams.put("user_name", FftEscalationApp.authUserName);
		authEngineparams.put("password", FftEscalationApp.authPassword);
		authEngineparams.put("application_id", FftEscalationApp.applicationID);
	...
	public String getThirdPartyAuthToken() {
		...
		String authThirdPArtyEngineUrl = FftEscalationApp.authThirdPartyApiUrl;
		/**
		 * Initialize the HashMap for storing parameters to get Auth Token.
		 */
		HashMap<String, String> authEngineparams = new LinkedHashMap<String, String>();
		...
		String authThirdTokenString = accessRestAPI.callingRestAPI(authThirdPArtyEngineUrl, authEngineparams);
		/**
		 * Retrieving the auth token from the response of API.
		 */

		String authThirdPartyToken = getAccessToken(authThirdTokenString);

		System.out.println("Token : " + authThirdPartyToken);
		return authThirdPartyToken;
	}
