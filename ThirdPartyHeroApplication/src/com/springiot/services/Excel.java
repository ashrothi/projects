...
public class Excel {
	...
	public String createExcel(XSSFWorkbook workbook, String reportName, String fileLocation) {
		...
		try {
			...
			/*
			 * Check if file exit if not than create directory
			 */
			if (!file.exists()) {
				file.mkdirs();
			}
			...
			out.close();
			/*
			 * Successful file written message
			 */
			...
			/*
			 * Handle exception if occur in process
			 */
			e.printStackTrace();
			return null;
		}
	...
	public boolean setSheetData(XSSFWorkbook workbook, String sheetName, List<Map<String, Object>> dataList,
		...
		try {
			...
			for (int i = 0; i < dataList.size(); i++) {
				...
				/*
				 * Get object of List of index i
				 */
				Map<String, Object> maplist = (Map<String, Object>) dataList.get(i);
				/*
				 * Initializing cell count
				 */
				int j = 0;
				/*
				 * to get each key value to insert in cell according to their
				 * datatype
				 */
				for (String key : maplist.keySet()) {
					/*
					 * Get String value of each object
					 */
					String valueRes = String.valueOf(maplist.get(key));
					/**
					 * Inserting data into cell according to their data type
					 */
					try {
