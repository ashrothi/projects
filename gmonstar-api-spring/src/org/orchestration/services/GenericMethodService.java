...
public class GenericMethodService {
	 ...
	 * @throws Exception
	 */
	static String isTokenRequired = "api_is_token_required";
	...
	public List<Map<String, Object>> tokenValid(List<Map<String, Object>> urlResponse,
		...
		//
		// List<Map<String, Object>> transformedDataList =
		// dataTransformater(urlResponse, requestParameterMap,
		// request, response);
		...
		for (Map<String, Object> apiMap : urlResponse) {
			try {
				...
				if (String.valueOf(apiMap.get(isTokenRequired)).equalsIgnoreCase("true")) {
					/**
					...
					if (tokenMessage.isValid()) {
						...
						if (tokenResponse.size() > 0) {
							...
							if (String.valueOf(tokenResponse.get(0).get("token_status")).equalsIgnoreCase("2")) {
								...
								for (String token : string) {
									...
									requestParameterMap.put(token, String.valueOf(tokenResponse.get(0).get("token")));
								}
						...
						if (tokenResponse.isEmpty()
							...
							String[] parameterDataGMParameterValue = String
									.valueOf(apiMap.get("authorization_parameters_value")).split(",");
							...
							String[] isQueryParameterValue = String
									.valueOf(apiMap.get("authorization_parameters_is_query")).split(",");
							String[] isPathParameterValue = String
									.valueOf(apiMap.get("authorization_parameters_is_path")).split(",");
							String[] isBodyParameterValue = String
									.valueOf(apiMap.get("authorization_parameters_is_body")).split(",");
							...
							for (int i = 0; i < inputParameterName.length; i++) {
								tokenInputParam.put(inputParameterName[i], parameterDataGMParameterValue[i]);

								if (isQueryParameterValue[i].equalsIgnoreCase("1")) {

									builder.append(String.valueOf(inputParameterName[i]) + "="
											+ String.valueOf(parameterDataGMParameterValue[i]) + "&");

								}
								if (isHeaderParameterValue[i].equalsIgnoreCase("1")) {
									headerParameterMap.put(String.valueOf(inputParameterName[i]),
											String.valueOf(parameterDataGMParameterValue[i]));
								}
							}

							if (builder.length() > 0) {
								String newUrl = String.valueOf(apiMap.get("api_token_api"));
								apiMap.remove("api_token_api");
								builder.deleteCharAt(builder.lastIndexOf("&"));
								apiMap.put("api_token_api", newUrl + "?" + builder.toString());

							}
							if (isPathParameterValue.toString().contains("1")
									|| isBodyParameterValue.toString().contains("1")) {

								Pattern pattern = Pattern.compile("<.+?>");
								StringBuffer url = new StringBuffer();
								Matcher matcher = pattern.matcher(String.valueOf(apiMap.get("token_api_template")));
								Matcher matcherUrl = pattern.matcher(String.valueOf(apiMap.get("api_token_api")));
								...
								if (matcherUrl.find()) {
									...
									if (tokenInputParam.containsKey(match_case_url.replaceAll("[<,>]", ""))) {
										match_case_value = String
												.valueOf(tokenInputParam.get(match_case_url.replaceAll("[<,>]", "")));
									...
									apiMap.put("api_token_api", String.valueOf(url));
								}
								...
								if (sb.length() == 0) {
									while (matcher.find()) {
										...
										// if (matchCase.replaceAll("[<,>]", "").contains("#&#")) {
										// String matchCaseValue = String
										// .valueOf(requesParameterMap.get(matchCase.replaceAll("[<,>]", "")));
										// matcher.appendReplacement(sb, matchCaseValue);
										// }else
										if (tokenInputParam.containsKey(matchCase.replaceAll("[<,>]", ""))) {
											String matchCaseValue = String
													.valueOf(tokenInputParam.get(matchCase.replaceAll("[<,>]", "")));
											matcher.appendReplacement(sb, matchCaseValue);
										} else {
							...
							String urlPassingParameter = String.valueOf(sb);
							...
							Map<String, String> authToken = getToken(hostUrl, urlPassingParameter, request, response,
									passingMap, String.valueOf(apiMap.get("token_api_method_type")),
									String.valueOf(apiMap.get("authorization_parameters_data_type")),
									headerParameterMap);
							/**
	...
	public ResponseEntity<?> genericExecuteApiMethod(String groupID, Map<String, String> parameterMap,
		...
		try {
			 ...
			 */
			inputParameterMap.put("api_group_id", String.valueOf(groupID));
			...
			if (getUrlsMessage.isValid()) {
				...
				if (!urlResponse.isEmpty()) {
					...
					if (urlResponse.get(0).get("api_group_type").toString().equalsIgnoreCase("ASYNC")) {
						groupLogID = urlResponse.get(0).get("api_group_id").toString();
	...
	private ResponseEntity<?> getSyncRequestData(List<Map<String, Object>> urlResponse,
		...
		if (validatedParameterList.size() == 0) {
			...
			for (Map<String, Object> map : transformedDataList) {
				...
				String hostUrl = String.valueOf(map.get("api_host_address")).concat(String.valueOf(map.get("api_url")));
				...
				if (passindMAp != null) {
					...
					if (String.valueOf(map.get("api_api_type")).equalsIgnoreCase("REST")) {
						if (headerParameterMap != null) {
							...
							// }
							//
							// String newhostUrl = hostUrl.concat("?" + builder);
							...
							responseFromEndNode = urlCalling.getOrchestrationData(hostUrl,
									...
									map.get("api_body_parameter_type").toString());
						} else {
							/**
				...
				Map<String, String> endNodeResponseMap = new LinkedHashMap<>();
				endNodeResponseMap = new LinkedHashMap<>();
				...
				OrchestrationMessage orchestrationMessage = orchestrationGenericProcess
						.GenericOrchestrationProcedureCalling("11", endNodeResponseMap, null, request, response);
				if (orchestrationMessage.isValid()) {
					...
					if (responseCode.equalsIgnoreCase("200") || responseCode.equalsIgnoreCase("202")) {
						if (endNodeResponse.contains("error")) {

							JsonOlcoreModification.parseOL(endNodeResponse, endNodeResponseMap);

							responseCode = endNodeResponseMap.get("error");

							switch (orchestrationMessageResponse.size()) {
							case 1:
								if (String.valueOf(
										orchestrationMessageResponse.get(0).get("notification_template_is_response"))
										.equalsIgnoreCase("1")) {

									return genericApiResponseMethod(String.valueOf(map.get("api_group_id")),
											endNodeResponse, orchestrationMessageResponse.get(0), request, response);

								} else {

									inputParameterMap.put("api_id", map.get("api_id").toString());
									return genericApiFailureNotifactionMethod(String.valueOf(map.get("api_group_id")),
							...
							case 2:
								...
								ResponseEntity<?> notificatPushStatus = genericApiFailureNotifactionMethod(
										String.valueOf(map.get("api_group_id")), inputParameterMap, request, response);
								if (notificatPushStatus.getStatusCode().is2xxSuccessful()) {
									return genericApiResponseMethod(String.valueOf(map.get("api_group_id")),
											endNodeResponse, orchestrationMessageResponse.get(1), request, response);
								} else {
									return notificatPushStatus;
								}

							default:
								return new ResponseEntity<>(String.valueOf(endNodeResponse), HttpStatus.ACCEPTED);
							}

						} else if (endNodeResponse.contains("responseCode")) {

							JsonOlcoreModification.parseOL(endNodeResponse, endNodeResponseMap);

							responseCode = endNodeResponseMap.get("responseCode");
							inputParameterMap.put("api_id", String.valueOf(map.get("api_id")));
							inputParameterMap.put("response_code", responseCode);
						...
						auditLogInsert(String.valueOf(inputParameterMap.get("tracking_message_header")),
								...
								String.valueOf(responseFromEndNode).replaceAll(",", "|:").replaceAll("\'", "\\\\'"),
								request);
						Map<String, Object> errorMessageMap = (Map<String, Object>) errorMAp.get("3");
						...
						errorMessage.remove("priority");
						errorMessage.put("code", errorMessage.get("code"));
						errorMessage.put("description", String.valueOf(responseFromEndNode.toString().substring(16)));
				...
				auditLogInsert(String.valueOf(inputParameterMap.get("tracking_message_header")),
						String.valueOf(map.get("api_group_id")), String.valueOf(map.get("api_id")),
						Integer.parseInt(String.valueOf(environment.getProperty("success.bit"))), "Sync API Calling",
			...
			return new ResponseEntity<>(finalErrorMessageMap, HttpStatus.INTERNAL_SERVER_ERROR);
			// return new
			// ResponseEntity<>(String.valueOf(responseFromEndNode.toString().substring(16)),
			// HttpStatus.ACCEPTED);
			...
			//// "Invalid Parameter :"+
			auditLogInsert(String.valueOf(inputParameterMap.get("tracking_message_header")), groupLogID, null,
					Integer.parseInt(String.valueOf(environment.getProperty("failure.bit"))), "Invalid Parameter ",
					String.valueOf(environment.getProperty("log.status.response.failure")),
					"Invalid Parameter API Response ",
					String.valueOf(validatedParameterList).replaceAll(",", "|:").replaceAll("\'", "\\\\'"), request);
			if (validatedParameterList.containsKey("mandatoryParameterMap")) {
				...
				errorMessage.remove("priority");
				errorMessage.put("code", errorMessage.get("code"));
				errorMessage.put("description", String.valueOf(validatedParameterList.get("mandatoryParameterMap"))
	...
	private ResponseEntity<?> genericApiResponseMethod(String valueOf, String endNodeResponse, Map<String, String> map,
			HttpServletRequest request, HttpServletResponse response) {
		List<Map<String, Object>> finalResponseList = new LinkedList<>();
		try {

			if (String.valueOf(map.get("notifiation_template_template")).equalsIgnoreCase("null")) {
				StringBuffer sb = new StringBuffer();
				if (String.valueOf(map.get("set_response_key")).equalsIgnoreCase("null")
					...
					Pattern pattern = Pattern.compile("<.+?>");
					Matcher matcher = pattern.matcher(String.valueOf(map.get("notifiation_template_template")));
				...
				Map<String, Object> templateMapParam = new Gson()
						.fromJson(String.valueOf(map.get("notifiation_template_template")), listType);
				List<Map<String, Object>> templateOfferList = (List<Map<String, Object>>) templateMapParam
						.get(String.valueOf(map.get("set_response_key")));

				Map<String, Object> apiResponse = new LinkedHashMap<>();
				apiResponse = new Gson().fromJson(String.valueOf(endNodeResponse), listType);
				/**
				 * Casting externalExtraPasses field from apiResponse in List<Map<String,
				 * Object>>
				 */

				logger.info("Response From Bss Node " + endNodeResponse);
							...
							if (value.contains(",")) {
								...
								for (String string : newValue) {
									...
									if (map2.containsKey(string.trim())) {
										finalValue.append(map2.get(string.trim()) + " ");
									} else {
										finalValue.append("");
									}
				...
				logger.info("Response after manipulation and modification...... " + apiResponse);

				return new ResponseEntity<>(new Gson().toJson(apiResponse), HttpStatus.ACCEPTED);
			}
		} catch (Exception e) {
			e.printStackTrace();
			Map<String, Object> apiResponse = new LinkedHashMap<>();
	...
	private ResponseEntity<?> getAsyncRequestData(List<Map<String, Object>> urlResponse,
			Map<String, String> inputParameterMap, String groupID, Map<String, Object> errorMAp,
			Map<String, Object> responseMap, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
			...
			if (!transformedDataList.isEmpty()) {
				...
				if (kafkaStatus) {
					...
					auditLogInsert(inputParameterMap.get("tracking_message_header"),
							...
							Integer.parseInt(String.valueOf(environment.getProperty("success.bit"))), "Pushed to kafka",
							String.valueOf(environment.getProperty("log.status.kafka.execution.success")), "template",
							String.valueOf(transformedDataList).replaceAll(",", "|:").replaceAll("\'", "\\\\'")
									.replaceAll("\'", "\\\\'"),
							request);
					/**
					 * Response Of API
				...
				auditLogInsert(inputParameterMap.get("tracking_message_header"), groupLogID, null,
						...
						String.valueOf(transformedDataList).replaceAll(",", "|:").replaceAll("\'", "\\\\'"), request);
				/**
				 * Response Of API
				 */
	...
	private Map<String, String> getToken(String hostUrl, String urlPassingParameter, HttpServletRequest request,
			...
			Map<String, String> headerParameterMap) throws Exception {
		/**
		...
		try {
			...
			if (newToken.toString().contains("responseCode:200")) {
				// String responseString = String.valueOf(newToken.toString().substring(16));
				...
				// .substring(responseString.indexOf(responseParameter[i]) +
				// responseParameter[i].length() + 3);
				//
				...
				logger.info("authResponseMap ------------------ " + authResponseMap);
			...
			logger.info("authResponseMap ------------------ " + passingMap.get("token"));
			/**
	...
	private Map<String, String> validate(String parameterValue, String parameterDatalength, String parameterDataRegex,
		...
		String errorMessage = "- Value not supported";
		try {
			...
			if (!parameterDatalength.equalsIgnoreCase("null")) {
				...
				if (parameterDatalength.contains(">")) {
					...
					if (parameterValue.length() <= length) {
						validationMap.put(parameterDataGMParameter, errorMessage);
	...
	public List<Map<String, Object>> dataTransformater(List<Map<String, Object>> tokenResponse,
			Map<String, String> requestParameter, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		/**
		...
		for (Map<String, Object> apiMap : tokenResponse) {
			try {
				...
				String[] isPathParameterValue = String.valueOf(apiMap.get("input_parameters_is_path")).split(",");
				String[] isBodyParameterValue = String.valueOf(apiMap.get("input_parameters_is_body")).split(",");
				...
				for (int i = 0; i < inputParameterName.length; i++) {
					...
					if (isQueryParameterValue[i].equalsIgnoreCase("1")) {
						if (!parameterDataGMParameterValue[i].equalsIgnoreCase("null")) {
							...
							builder.append(String.valueOf(inputParameterName[i])
									.concat("=" + String.valueOf(apiMap.get(parameterGMParameter[i])) + "&"));
						} else {
					...
					if (isPathParameterValue[i].equalsIgnoreCase("1")) {
						...
						if (matcherUrl.find()) {
							...
							if (match_case_url.replaceAll("[<,>]", "").contains("#&#")) {
								match_case_value = String
										.valueOf(requestParameter.get(match_case_url.replaceAll("[<,>]", "")));

							}
							if (requestParameter.containsKey(match_case_url.replaceAll("[<,>]", ""))) {
								match_case_value = String
										.valueOf(requestParameter.get(match_case_url.replaceAll("[<,>]", "")));
							...
							matcherUrl.appendReplacement(url, match_case_value);
							requestParameter.put("api_url", String.valueOf(url));
						}
				...
				apiMap.put("requestId", String.valueOf(requestParameter.get("requestId")));
	...
	public Map<String, Object> getUrlParameters(Map<String, Object> apiMap, Map<String, String> requesParameterMap,
		...
		try {
			...
			// String.valueOf(apiMap.get("api_response_parameter_value")).split(",");
			// String[] isQueryParameterValue =
			// String.valueOf(apiMap.get("input_parameters_is_query")).split(",");
			//
			// String[] isBodyParameterValue =
			// String.valueOf(apiMap.get("input_parameters_is_body")).split(",");
			// Map<String, Object> parameterMap = new LinkedHashMap<>();
			/**
			...
			for (int i = 0; i < inputParameterName.length; i++) {
				...
				if (isHeaderParameterValue[i].equalsIgnoreCase("1")) {
					headerMap.put(inputParameterName[i], inputParameterValue[i]);
				} else {
					bodyMap.put(inputParameterName[i], inputParameterValue[i]);
				}
			...
			if (String.valueOf(apiMap.get("api_template")).equalsIgnoreCase("null")) {
				if (String.valueOf(apiMap.get("api_api_type")).equalsIgnoreCase("REST")) {
					...
					if (matcherUrl.find()) {
						...
						// if (match_case_url.replaceAll("[<,>]", "").contains("#&#")) {
						// match_case_value = String
						// .valueOf(bodyMap.get(match_case_url.replaceAll("[<,>]", "")));
						//
						// }
						if (requesParameterMap.containsKey(match_case_url.replaceAll("[<,>]", ""))) {
							match_case_value = String.valueOf(bodyMap.get(match_case_url.replaceAll("[<,>]", "")));
				...
				parameterMAp.put("body_parameter", "");
				parameterMAp.put("header_parameter", headerMap);
			}
	...
	public Map<String, Object> getErrorCodes(String groupID, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (groupID.contains(",")) {
				...
				passinMap.put("api_group_id", String.valueOf(newGroupId));
			} else {
	...
	public ResponseEntity<?> checkRequestId(String groupID, Map<String, String> parameterMap,
		...
		try {
			...
			if (requestIdValidationResponse.get(0).get("is_exists").toString().equalsIgnoreCase("1")) {
				...
				errorMessage.put("code", errorMessage.get("code"));
				errorMessage.put("description",
						errorMessage.get("description").toString().concat(":" + "Pending Request Already Exists"));
				List<Map<String, Object>> errorList = new LinkedList<>();
				errorList.add(errorMessage);
				Map<String, Object> finalErrorMessageMap = new LinkedHashMap<>();
				finalErrorMessageMap.put("errors", errorList);
	...
	public ResponseEntity<?> getOperatorConfig(Map<String, String> parameterMap, HttpServletRequest request,
		...
		Map<String, Object> errorMAp = getErrorCodes(environment.getProperty("router.config.name"), request, response);
		try {

			parameterMap.put("api_group_id", String.valueOf(environment.getProperty("router.config.name")));
			...
			if (!getOperatorResponse.isEmpty()) {
				...
				ResponseEntity<?> requestRouterResponse = getSyncRequestData(getOperatorResponse, parameterMap,
						environment.getProperty("router.config.name"), errorMAp, request, response);
				if (requestRouterResponse.getStatusCode().is2xxSuccessful()) {
					...
					System.out.println(getPlatform.getObject());
	...
	public ResponseEntity<?> genericApiFailureNotifactionMethod(String groupId, Map<String, String> parameterMap,
			HttpServletRequest request, HttpServletResponse response) throws Exception {
		/**
		 * Initializing the response message
		 */
		...
		try {
			 ...
			 * validate the user.
			 */
			OrchestrationMessage message = orchestrationGenericProcess.GenericOrchestrationProcedureCalling("11",
					parameterMap, null, request, response);
			List<Map<String, Object>> templateRespoonse = (List<Map<String, Object>>) message.getObject();
			/**
			 * To response is Valid
			 */
			if (message.isValid()) {
				...
				Matcher matcher = pattern
						.matcher(String.valueOf(templateRespoonse.get(0).get("notifiation_template_template")));

				if (sb.length() == 0) {
					while (matcher.find()) {
						String matchCase = matcher.group(0);

						if (parameterMap.containsKey(matchCase.replaceAll("[<,>]", ""))) {
							String matchCaseValue = String.valueOf(parameterMap.get(matchCase.replaceAll("[<,>]", "")));
							matcher.appendReplacement(sb, matchCaseValue);
						}
