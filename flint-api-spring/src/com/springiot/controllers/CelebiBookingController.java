...
import com.springiot.swagger.response.FlintGetCustomerUserTagSwagger;
...
public class CelebiBookingController {
	...
	@ApiOperation(value = "/flint/city/get/by/parentId", notes = "To get the sub city of given parent city", response = FlintGetCustomerUserTagSwagger.class)
	@ApiImplicitParams({
		@ApiImplicitParam(name = "city_id", value = "Requires city id", required = true, access = "query", paramType = "query", dataType = "String") })
	/*
	 * Input Parameters are map,request and response.Map handles all manipulation in
	 * data received from procedure
	 */

	@RequestMapping(value = "/flint/city/get/by/parentId", method = RequestMethod.POST)
	public @ResponseBody Message flintcitygetbyparentId(@ApiIgnore @RequestParam Map<String, String> map,
			HttpServletRequest request, HttpServletResponse response) throws Exception {
		/*
		 * Method GenericProcedureCalling() is called upon to get the data from
		 * respective database.
		 */
		return genericProcess.GenericProcedureCalling("121", map, null, request, response);
	}
}
