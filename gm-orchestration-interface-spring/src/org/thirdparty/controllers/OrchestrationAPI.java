...
public class OrchestrationAPI {
	...
	public ResponseEntity<?> deviceRegister(@PathVariable String deviceId,
		...
		ResponseEntity<?> responseMessage = thirdPartyService.simProfiles(new Gson().toJson(simProfileData).toString(),
	...
	/**
	 * This API is Service to update settings on a device.1.Enable voice
	...
	public ResponseEntity<?> deviceCreate(@PathVariable String deviceId,
		...
		ResponseEntity<?> responseMessage = thirdPartyService
				.deviceSettings(new Gson().toJson(deviceSettingData).toString(), deviceId, request, response);
	...
	public ResponseEntity<?> onBoarding(
		...
		ResponseEntity<?> responseMessage = thirdPartyService
				.onBoarding(new Gson().toJson(onbordingRequestData).toString(), request, response);
	...
	public ResponseEntity<?> updateSubscriber(@PathVariable String ban,
		...
		ResponseEntity<?> responseMessage = thirdPartyService
				.updateSubscriber(new Gson().toJson(updateSubscriberRequestData).toString(), ban, request, response);
	...
	public ResponseEntity<?> offBoarding(@PathVariable String ban, @PathVariable() Optional<String> deviceId,
		...
		ResponseEntity<?> responseMessage = thirdPartyService.offBoarding(deviceId, ban, request, response);

		return responseMessage;
	}

	/**
	 * This API is used to get order details on a specific account including
	 * data usage.IncludeCanceled = true; returns details of all Active, Queued,
	 * Canceled, and Expired orders.IncludePending = true; returns details of
	 * any pending orders.Data usage details of the active order will always be
	 * returned if there is an active order
	 * 
	 * @param ban
	 *            billing account number created for the subscriber/device
	 * @param request:::To
	...
	public ResponseEntity<?> getSubscriptionDetail(@PathVariable String ban, HttpServletRequest request,
		...
		ResponseEntity<?> responseMessage = thirdPartyService.getSubscriptionDetail(ban, request, response);
	...
	public ResponseEntity<?> getOffer(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ResponseEntity<?> responseMessage = thirdPartyService.getOffer(request, response);
	...
	public ResponseEntity<?> addOrder(@PathVariable String ban,
		...
		ResponseEntity<?> responseMessage = thirdPartyService.addOrder(ban, new Gson().toJson(addOrder).toString(),
				request, response);
	...
	@ApiImplicitParams({
			...
			@ApiImplicitParam(name = "returnUrl", value = "URL to send async response. ", required = true, access = "header", paramType = "header", dataType = "String"),
			@ApiImplicitParam(name = "ban", value = "Requires billing account number created for the subscriber/device.", required = true, access = "path", paramType = "path", dataType = "String"),
			@ApiImplicitParam(name = "orderId", value = "Requires unique order id which has to be cancelled.", required = true, access = "path", paramType = "path", dataType = "String") })

	@RequestMapping(value = "/subscribers/{ban}/orders/{orderId}", method = RequestMethod.DELETE)
	public ResponseEntity<?> addOrder(@PathVariable String ban, @PathVariable String orderId,
			HttpServletRequest request, HttpServletResponse response) throws Exception {

		ResponseEntity<?> responseMessage = thirdPartyService.cancelOrder(ban, orderId, request, response);
	...
	public ResponseEntity<?> pushNotification(@RequestParam Object data, @RequestParam String kafka_type,
		...
		ResponseEntity<?> responseMessage = thirdPartyService.pushNotification(data, kafka_type, request, response);
	...
	public ResponseEntity<?> gcProfileSwitch(@RequestParam String data, HttpServletRequest request,
		...
		ResponseEntity<?> responseMessage = thirdPartyService.gcProfileSwitch(data, request, response);
	...
	public ResponseEntity<?> onboardingFromEsimNotification(@RequestParam String data, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		// System.out.println("Onboarding through Esim :::::::::: Request data
		// :----" + data);
		ResponseEntity<?> responseMessage = thirdPartyService.onboardingFromEsimNotification(data, request, response);
